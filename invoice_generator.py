...
class InvoiceGenerator:
    ...
    def _draw_background_image(self, page, image_path, opacity=1.0):
        ...
        try:
            ...
            if opacity < 1.0:
                # اطمینان از تبدیل به RGBA برای اعمال شفافیت
                if img_pil.mode != 'RGBA':
                    img_pil = img_pil.convert('RGBA')
                # جدا کردن کانال آلفا و اعمال شفافیت
    ...
    def _draw_header_image(self, page, image_path):
        if not os.path.exists(image_path):
            ...
            return
        try:
            img_pil = Image.open(image_path) # اینجا مسیر فایل (رشته) استفاده میشه
            header_height = 100
            header_width = int(img_pil.width * (header_height / img_pil.height))
            
            if header_width > page.rect.width - 20: 
            ...
            self.y_cursor = page.rect.height - (y_pos + header_height + 20) 
        except Exception as e:
            ...
            self.y_cursor = page.rect.height - 120 
    ...
    def _draw_footer_image(self, page, image_path):
        ...
        try:
            ...
            if footer_width > page.rect.width - 20: 
            ...
            y_pos = page.rect.height - footer_height - 10 
    ...
    # ... (بقیه کلاس InvoiceGenerator و بلاک if __name__ == "__main__": بدون تغییر)
    ...
    def _draw_header_text_only(self, invoice_data, customer_data, invoice_template): 
        ...
        if logo_path and os.path.exists(logo_path):
            try:
                logo_rect = fitz.Rect(50, self.y_cursor + 40, 150, self.y_cursor + 90)
    ...
    def _draw_invoice_items_table(self, invoice_items):
        table_start_y = self.y_cursor - 10 
        table_x0 = 50
        table_x1 = self.page.rect.width - 50
        col_x_positions = [table_x1 - 80, 
                           table_x1 - 80 - 80, 
                           table_x1 - 80 - 80 - 200, 
                           table_x1 - 80 - 80 - 200 - 100, 
                           table_x1 - 80 - 80 - 200 - 100 - 50] 
        ...
        self.y_cursor = current_y - 20 

    def _draw_totals(self, invoice_data):
        total_x_align = self.page.rect.width - 60 
        line_height = 20
...
if __name__ == "__main__":
    ...
    dummy_settings = AppSettings(
        ...
        # مسیر لوگوی تست را اینجا وارد کنید
    ...
    dummy_template = InvoiceTemplate(
        ...
        is_active=1,
        # مسیرهای نمونه برای تست - مطمئن شوید این فایل‌ها در کنار invoice_generator.py وجود دارند
    ...
    if success:
        ...
        try:
            if sys.platform == "win32":
                subprocess.run(["start", pdf_path], shell=True) # Changed to 'start' for better opening on Windows
